...
// DAO 数据访问对象;主要是包含一些增删改查方法, 用来操作数据库,进而实现对题目的相关操作~~
public class ProblemDAO {
    ...
    // 查找全部题目(用来实现题目列表页)
    // 只需要查找 Problem 的一部分字段即可：id，title，level这三个部分
    // 正常来说, 一个系统的 "查找全部操作" 一般都得支持分页查询。不分页的话, 数据量可能会比较大, 影响程序的效率
    // 当前并不考虑实现分页查询. 后端实现分页是非常容易的~~
    // SQL 中本来就有 limit offset 这样的基础支持，但是前端实现分页不太好写
    ...
    // 查找指定题目(用来实现题目详情页)，需要把 Problem 每个字段都查询出来
    public Problem selectOne(int problemId) {
        ...
        try {
            statement = connection.prepareStatement(sql);
            ((PreparedStatement) statement).setInt(1, problemId);
            // 3. 执行 SQL
            resultSet = statement.executeQuery();
